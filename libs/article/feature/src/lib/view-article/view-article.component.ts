...
export class ViewArticleComponent implements OnInit {
  ...
  async loadArticle(slug: string) {
    try {
      let res = await this.articleService
        .getOne(slug)
        .pipe(take(1))
        .toPromise();
  ...
  async loadComments() {
    const res = await this.commentService
      .getAllComments(this.article?.slug)
      .pipe(take(1))
      .pipe(take(1))
      .toPromise();
  ...
  async postComment() {
    const res = await this.commentService
      .postComment(this.article?.slug, this.commentForm.value)
      .pipe(take(1))
      .toPromise();
  ...
  async deleteComment(id: string) {
    await this.commentService
      .deleteComments(this.article?.slug, id)
      .pipe(take(1))
      .toPromise();
  ...
  async toggleFollow($event: boolean) {
    ...
    if ($event) {
      promise = this.profileService
        .followAUser(this.article?.author?.username)
        .pipe(take(1))
        .toPromise();
    } else {
      promise = this.profileService
        .unfollowAUser(this.article?.author?.username)
        .pipe(take(1))
        .toPromise();
    }
